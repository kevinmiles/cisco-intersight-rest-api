...
### Import the Intersight REST API Postman Collection
...
2. View the contents of the Collection
  ...
  **What it does:** Convert the `responseBody` from the **POST** to JSON and set the Postman global variable `ntp-policy` to the `Moid` of the just created `NTP Policy`.
  ...
  **What it does:** Unset the Postman global variable `ntp-policy`. Removing the `Moid` of the NTP Policy created with the POST and updated by the PATCH.
