...
This Introduction to the Intersight REST API with Postman Learning Lab will guide you through using Postman to perform queries, uUpdates and configuration with Intersight.
...
## Prerequisites
Prior to starting this learning lab, it helps to understand of basic Intersight functionality.
...
**What it does:** Unsest the Postman global variable `ntp-policy`. Removing the `Moid` of the NTP Policy created with the `POST` and updated by the `PATCH`.
